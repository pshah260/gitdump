...
def m2():
    ...
    df['TotalPTS'] = df['Runs']*1 + df['4s']*1 + df['6s']*2 + df['50']*8 + df['100']*16 + df['Wkts']*25 + df['4w']*8 + df['5w']*16 + df['Catches']*8 + df['Stumpings']*16 + df.apply(h, axis=1)*4 + df['Inns_y']*df.apply(f,axis=1) + df['Inns_x']*df.apply(g,axis=1)
